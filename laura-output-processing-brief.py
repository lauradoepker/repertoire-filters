...
print 'index    genes                                   size  n muts  SHM   rep frac  CDR3                           SFS'
def print_stuff(line):
    ...
    print '%4s  %13s %16s %10s %5d %5d %7.3f %8.4f   %-30s %4.2f' % (cluster_index, line['v_gene'], line['d_gene'], line['j_gene'], len(line['unique_ids']), numpy.mean(line['n_mutations']), numpy.mean(line['mut_freqs']), float(len(cluster)) / n_total, Seq(cdr3_seq).translate(), utils.fay_wu_h(line, debug=False))
#    print line['v_gene'],
#    print line['d_gene'],
#    print line['j_gene']
...
#    print utils.fay_wu_h(line, debug=False), 'SFS (Fay Wu H) score' # site frequency spectrum
